...
* The remainder of the necessary components, as described in
  <<anatomy.adoc,anatomy>> are created, including the replication controller
  and service.
...
Strategy: Docker
URL: https://github.com/jdob-openshift/python-web <1>
From Image: ImageStreamTag openshift/python:latest <2>
Output to: ImageStreamTag python-web:latest <3>
...
Webhook Generic:
    ...
    AllowEnv:	false
----
<1> The `URL` value corresponds to the git repository where the source is found.
<2> The strategy is set to Docker, indicating that the image should be built
...
Strategy: Source
URL: https://github.com/jdob-openshift/python-web-source <1>
From Image: ImageStreamTag openshift/python:3.4 <2>
Output to: ImageStreamTag python-web-source:latest <3>
...
----
<1> The `URL` value corresponds to the git repository where the source is found.
<2> The strategy is set to Source, indicating that the image should be built
...
Now that an application is deployed, see the <<anatomy.adoc,anatomy>> section
guide for more information on the different resources that were created, or the
<<basic-usage.adoc,basic usage>> guide for other ways to interact with the newly
deployed application. Information on enhancing container images to utilize
OpenShift's features can be found in the <<integration.adoc,integration>> guide.
