...
this guide can be found in the <<basic-usage#resource-query-commands>> section.
====

When applications are deployed, either directly from an image or built using
<<terminology#term-source-to-image,source to image>>, there are a number of
resources that OpenShift creates to support the application.

=== Image Stream

If an existing <<terminology#image-stream,image stream>> is not found, OpenShift
...
in an image stream contents and roll out updated pods.
...
A <<terminology#rc,replication controller>> is created when an application is
deployed and is used to control the number of running pods. Each application
deployment gets its own replication controller.
...
The next level up is the <<terminology#dc,deployment configuration>> which describes
...
The last layer of interest is the created <<terminology#service,service>>. The
service acts as the entry point into the running application, taking care of
distributing requests to the appropriate pod.
